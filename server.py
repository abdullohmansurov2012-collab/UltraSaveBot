...
@app.get("/script.js")
async def read_js(): return FileResponse('script.js')
...
def extract_video_info(url: str):
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                for f in reversed(info['formats']):
                    ...
                    if h in ALLOWED_HEIGHTS:
                        ...
                        if res_str not in seen_resolutions:
                            qualities.append({"height": res_str, "url": f.get('url'), "ext": f.get('ext', 'mp4'), "size": round(f.get('filesize', 0) / (1024 * 1024), 1) if f.get('filesize') else "Noma'lum"})
            ...
            return {"status": "success", "title": info.get('title', 'Video'), "thumbnail": info.get('thumbnail', ''), "platform": info.get('extractor', 'vid').lower(), "qualities": qualities}
